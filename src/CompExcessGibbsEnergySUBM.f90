...
!    end do
    ...
    LOOP_Param: do abxy = nParamPhase(iSolnIndex-1) + 1, nParamPhase(iSolnIndex)
        ...
        a = iRegularParam(abxy,2)              ! Index of A -> i (anion)
        b = iRegularParam(abxy,3)              ! Index of B -> j (anion)
        x = iRegularParam(abxy,4)             ! Index of X -> pi
        y = iRegularParam(abxy,5)             ! Index of Y -> pj
!        x = xx - nSublatticeElements(iSPI,1)  ! Index of X
!        y = yy - nSublatticeElements(iSPI,1)  ! Index of Y
        p = iRegularParam(abxy,6)              ! Index of k (cation)
        q = iRegularParam(abxy,7)              ! Index lowercase-l (cation)
